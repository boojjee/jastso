...
module.exports = {
  ...
  index: function(req, res, next) {
    ...
    MongoClient.connect(sails.config.native_mongodb.url, function(err, db) {
      ...
      db.collection('school').findOne({ sc_code: criteria.sc_code }, function(err, schoolData){
        ...
        db.collection('teacher').find({ sc_code: criteria.sc_code }).toArray(function(err, teacherData){
          if(err){ console.log(err) };
    ...
    MongoClient.connect(sails.config.native_mongodb.url, function(err, db) {
      ...
      db.collection('school').findOne({ sc_code: criteria.sc_code }, function(err, schoolData){
        ...
        db.collection('teacher').findOne({ 
            sc_code: criteria.sc_code 
          , _id : ObjectId.createFromHexString(teacher_id) }, function(err, teacherData){
          if(err){ console.log(err) };
  ...
  timetable_edit: function(req, res, next) {
    ...
    _.each(dd, function (data) {
      ...
      tmp.push({ course_id: ObjectId(data.course_id), time: tmp_time_data} )
      tmp_time_data = [];
    })
    ...
    MongoClient.connect(sails.config.native_mongodb.url, function(err, db) { 
      db.collection('teacher_timetable').find({ teacher_id: ObjectId(teacher_id) }).toArray(function(err, courseData){
        if(err){ console.log(err) };
        ...
        if(_.isEmpty(courseData)){
          ...
          db.collection('teacher_timetable').insert(timetable_data, function(err, insertedtimetable_data) {
            ...
            res.redirect('/'+ criteria.sc_code +'/teacher')
          })
        }else{
          console.log("update");
          console.log(tmp);
